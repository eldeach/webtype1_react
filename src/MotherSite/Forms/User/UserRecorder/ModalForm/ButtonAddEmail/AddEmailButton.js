...
// ======================================================================================== [Import Component] js
import addEmailButtonLang from './addEmailButtonLang.js'
...
function AddEmail(props){
    ...
    const yupSchema = yup.object().shape({
        ...
        .email(addEmailButtonLang.inputField.email_address.valMsg.pattern[cookies.load('site-lang')]) // 이메일 유효성 검사
        .required(addEmailButtonLang.inputField.email_address.valMsg.required[cookies.load('site-lang')]), // 필수 입력 필드
      });
    ...
    const arrAddElement = function (arr, newElement) {
        ...
        if (duplication === 0){
            ...
            alert(addEmailButtonLang.alertMsg.duplicated[cookies.load('site-lang')])
        }
        return tempArr;
    };
                    ...
                    <Modal open={(popup === 1)} onClose={handleModalClose}>
                        <Paper sx={style.popup.paper} elevation={3}>
                            ...
                            >
                                ...
                                id="email_address"
                                name="email_address"
                                label={addEmailButtonLang.inputField.email_address.placeholder[cookies.load('site-lang')]}
                                ...
                                <FormControl style={{ border:'#D3D3D3 solid 1px', borderRadius:'5px', marginTop:'5px', marginBottom:'2px', paddingLeft:'10px', width:'100%', boxSizing:'border-box', display: 'flex', flexDirection: 'row', alignItems:'center' }}>
                                    <FormLabel id="email_usage" sx={{mr:1, fontSize:14}}>{addEmailButtonLang.inputField.email_usage.placeholder[cookies.load('site-lang')]}</FormLabel>
                                        <RadioGroup
                                        row
                                        ...
                                        >
                                            ...
                                            value="PERSONAL"
                                            control={<Radio />}
                                            label={<Typography sx={{fontSize:14}}>{addEmailButtonLang.inputField.email_usage.optionLabel.personal[cookies.load('site-lang')]}</Typography>}
                                            />
                                            <FormControlLabel
                                            value="WORK"
                                            control={<Radio />}
                                            label={<Typography sx={{fontSize:14}}>{addEmailButtonLang.inputField.email_usage.optionLabel.work[cookies.load('site-lang')]}</Typography>}
                                            />
                                            <FormControlLabel
                                            value="OTHER"
                                            control={<Radio />}
                                            label={<Typography sx={{fontSize:14}}>{addEmailButtonLang.inputField.email_usage.optionLabel.other[cookies.load('site-lang')]}</Typography>}
                                            />
                                        </RadioGroup>
                                </FormControl>
                                <TextField
                                ...
                                label={addEmailButtonLang.inputField.email_affiliation.placeholder[cookies.load('site-lang')]}
