...
// ======================================================================================== [Import Component] js
import addPhoneButtonLang from './addPhoneButtonLang.js'
...
function AddPositionButton(props){
    ...
    const yupSchema = yup.object().shape({
        ...
        .matches(/[0-9]/,addPhoneButtonLang.inputField.phone_number.valMsg.pattern[cookies.load('site-lang')])
        .required(addPhoneButtonLang.inputField.phone_number.valMsg.required[cookies.load('site-lang')]), // 필수 입력 필드
      });
    ...
    const arrAddElement = function (arr, newElement) {
        ...
        if (duplication === 0){
            ...
            alert(addPhoneButtonLang.alertMsg.duplicated[cookies.load('site-lang')])
        }
        return tempArr;
    };
                    ...
                    <Modal open={(popup === 1)} onClose={handleModalClose}>
                        <Paper sx={style.popup.paper} elevation={3}>
                            ...
                            >
                                ...
                                id="phone_number"
                                name="phone_number"
                                label={addPhoneButtonLang.inputField.phone_number.placeholder[cookies.load('site-lang')]}
                                ...
                                <FormControl style={{ border:'#D3D3D3 solid 1px', borderRadius:'5px', marginTop:'5px', marginBottom:'2px', paddingLeft:'10px', width:'100%', boxSizing:'border-box', display: 'flex', flexDirection: 'row', alignItems:'center' }}>
                                    <FormLabel id="phone_usage" sx={{mr:1, fontSize:14}}>{addPhoneButtonLang.inputField.phone_usage.placeholder[cookies.load('site-lang')]}</FormLabel>
                                        <RadioGroup
                                        row
                                        ...
                                        >
                                            ...
                                            value="PERSONAL"
                                            control={<Radio />}
                                            label={<Typography sx={{fontSize:14}}>{addPhoneButtonLang.inputField.phone_usage.optionLabel.personal[cookies.load('site-lang')]}</Typography>}
                                            />
                                            <FormControlLabel
                                            value="WORK"
                                            control={<Radio />}
                                            label={<Typography sx={{fontSize:14}}>{addPhoneButtonLang.inputField.phone_usage.optionLabel.work[cookies.load('site-lang')]}</Typography>}
                                            />
                                            <FormControlLabel
                                            value="OTHER"
                                            control={<Radio />}
                                            label={<Typography sx={{fontSize:14}}>{addPhoneButtonLang.inputField.phone_usage.optionLabel.other[cookies.load('site-lang')]}</Typography>}
                                            />
                                        </RadioGroup>
                                </FormControl>
                                <TextField
                                ...
                                label={addPhoneButtonLang.inputField.phone_affiliation.placeholder[cookies.load('site-lang')]}
